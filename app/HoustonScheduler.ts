...
export class HoustonScheduler implements Scheduler {
    ...
    readonly mapServer = 'https://mycity2.houstontx.gov/pubgis01/rest/services/SWM/SWM_Services/MapServer/';
    ...
    constructor(pos: PosCoords, numberOfDays: number = 60) {
        ...
        const mapLayers = {1: 'DAY', 2: 'SERVICE_DAY', 3: 'SERVICE_DA'};
        const requests = Object.keys(mapLayers).map(layerId => {
            return `${this.mapServer}${layerId}/query${paramStr}&outFields=${mapLayers[layerId]}`
        }).map(url => axios.request({method: 'GET', url, timeout: 15000, responseType: 'json'}))

        //get all relevant data
        this.whenLoaded = Promise.all(requests)
            .then(requests => requests.reduce((acc, r) => ({...acc, ...r.data.features[0].attributes}), {})
            ).then((results) => {
                console.log(results);
                this.parseData({
                    garbage: results.DAY,
                    recycling: results.SERVICE_DAY,
                    junk: results.SERVICE_DA,
                });
            });
    }
     ...
     * @returns {Array<any>}
     */
    parseData(results: { garbage: string, recycling: string, junk: string }) {
        ...
        console.log(results);
        if (this.isValidData(results)) {

            wasteDay = HoustonScheduler.getDayIndex(results.garbage);
            let junkPattern = results.junk;
            ...
            let recyclingSchedule = results.recycling;
